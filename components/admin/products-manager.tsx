    ...
    <>
      ...
      {initialProducts.length === 0 ? (
        <div className="border border-border bg-secondary/20 p-16 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{t("emptyHint")}</p>
        </div>
      ) : (
        <div className="space-y-12">
          {CATEGORIES.map((category) => {
            const items = initialProducts.filter((p) => p.category === category)
            if (items.length === 0) return null
            return (
              <section key={category}>
                <div className="mb-4 flex items-baseline gap-3 border-b border-border pb-3">
                  <h2 className="font-serif text-xl">{tCategories(category)}</h2>
...
function ProductCard({
  ...
  const tCategories = useTranslations("Categories")
  return (
    <article className="flex gap-4 border border-border bg-background p-4">
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex-1">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {t("stock")}: {p.stock}
            {!p.is_active && <span className="ml-2 text-destructive">· {t("hidden")}</span>}
          </p>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-foreground/30 p-4 backdrop-blur-sm">
      <div className="mt-10 w-full max-w-2xl bg-background shadow-2xl">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <h2 className="font-serif text-2xl">{product ? t("editTitle") : t("createTitle")}</h2>
        ...
        <form onSubmit={onSubmit} className="space-y-5 p-6">
          <div className="grid gap-5 sm:grid-cols-2">
            <Input label={t("fieldName")} name="name" defaultValue={product?.name} required />
            <Input label={t("fieldSlug")} name="slug" defaultValue={product?.slug} required />
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            <label className="block">
              <span className="mb-2 block text-[11px] tracking-[0.2em] text-muted-foreground uppercase">{t("fieldCategory")}</span>
              <select
                name="category"
            ...
            <Input label={t("fieldPrice")} name="price" type="number" step="0.01" defaultValue={product?.price?.toString()} required />
            <Input label={t("fieldStock")} name="stock" type="number" defaultValue={product?.stock?.toString() ?? "0"} required />
          </div>
          <div className="block">
            <span className="mb-2 block text-[11px] tracking-[0.2em] text-muted-foreground uppercase">{t("fieldPhoto")}</span>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                >
                  ...
                  {imgUploading ? t("uploading") : t("uploadPhoto")}
                </button>
                <input
                  type="text"
                  name="image_url"
          ...
          <Input label={t("fieldMaterial")} name="material" defaultValue={product?.material ?? "925 Sterling Silver"} />
          <label className="block">
            <span className="mb-2 block text-[11px] tracking-[0.2em] text-muted-foreground uppercase">{t("fieldDescription")}</span>
            <textarea
              name="description"
              rows={4}
